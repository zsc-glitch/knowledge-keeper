...
// 获取版本历史目录
export function getVersionDir(vaultDir: string, knowledgeId: string): string {
  return path.join(vaultDir, ".versions", knowledgeId);
}

// 确保版本目录存在
async function ensureVersionDir(vaultDir: string, knowledgeId: string): Promise<string> {
...
export async function saveVersion(
  ...
  source: "conversation" | "manual" | "rollback" = "conversation"
...
export async function listVersions(
  ...
  try {
    ...
    for (const line of lines) {
      try {
        const version = JSON.parse(line) as VersionRecord;
...
async function updateVersionIndex(
  ...
  let index: Record<string, VersionIndex> = {};
  try {
...
async function cleanupOldVersions(
  ...
  await fs.writeFile(
    versionFile,
    toKeep.map(v => JSON.stringify(v)).join("\n") + "\n",
    "utf-8"
  );
...
export async function cleanupExpiredVersions(
...
): Promise<{ knowledgeIds: number; versionsDeleted: number }> {
  const indexDir = path.join(vaultDir, ".versions");
  const indexPath = path.join(indexDir, "index.json");
  ...
  try {
    const content = await fs.readFile(indexPath, "utf-8");
    const index = JSON.parse(content) as Record<string, VersionIndex>;
    ...
    for (const [knowledgeId, info] of Object.entries(index)) {
      ...
      if (recentVersions.length === 0 && versions.length > 0) {
        ...
        await fs.writeFile(
          versionFile,
          recentVersions.map(v => JSON.stringify(v)).join("\n") + "\n",
          "utf-8"
        );
...
// 获取版本统计
export async function getVersionStats(vaultDir: string): Promise<{
