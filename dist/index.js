...
import { logAudit, readAuditLogs, verifyAuditIntegrity, cleanupAuditLogs, getAuditStats, } from "./audit.js";
import { saveVersion, listVersions, getVersion, cleanupExpiredVersions, getVersionStats, compareVersions, } from "./version.js";
// 错误类型
...
// 通过 ID 加载知识点
async function loadKnowledgeById(vaultDir, id) {
...
async function updateIndex(vaultDir, kp, mode = "add") {
    ...
    return indexLock = indexLock.then(async () => {
        ...
        let index = { version: 1, entries: [], tagsIndex: {} };
        try {
                ...
                try {
                    ...
                    if (params.links && params.links.length > 0) {
                        for (const linkId of params.links) {
                            ...
                            if (!exists) {
                                throw new KnowledgeError(`关联的知识点不存在: ${linkId}`, "LINK_NOT_FOUND", { linkId });
                            }
                        }
                    }
                    const kp = {
                    ...
                    for (const type of types) {
                        ...
                        try {
                            ...
                            for (const file of files) {
                                ...
                                const matchesQuery = kp.title.toLowerCase().includes(queryLower) ||
            ...
            async execute(toolCallId, params) {
                try {
                    ...
                    for (const linkId of newLinks) {
                        if (linkId === kp.id) {
                            throw new KnowledgeError("知识点不能关联自己", "SELF_LINK", { id: kp.id });
                        }
                        const exists = await findKnowledgeFile(vaultDir, linkId);
                        if (!exists) {
                            throw new KnowledgeError(`关联的知识点不存在: ${linkId}`, "LINK_NOT_FOUND", { linkId });
        ...
        api.registerTool({
            ...
            async execute(toolCallId, params) {
                try {
                    ...
                    // 重建索引
                    const types = ["concept", "decision", "todo", "note", "project"];
                    const newIndex = { version: 1, entries: [], tagsIndex: {} };
                    for (const type of types) {
                        const typeDir = path.join(vaultDir, getTypeDir(type));
                        try {
                            const files = await fs.readdir(typeDir);
